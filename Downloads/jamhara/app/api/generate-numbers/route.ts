...
import { buildNumbersPrompt } from "@/lib/prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (topic?.trim()) {
    const dupCheck = await checkTopicDuplicate(topic.trim(), "numbers", category_slug);
  ...
  const recentTopics = await getRecentTopics(category_slug, "numbers", 15, 30);
  const recentTopicsBlock = recentTopics.length > 0
    ...
    : "";
  const promptText = buildNumbersPrompt({
  ...
  try {
    ...
    if (!cfg?.stats?.length) return NextResponse.json({ error:"Invalid numbers config" }, { status:500 });
    ...
    const { data:post, error:err } = await supabase.from("posts").insert({
      ...
      type:"numbers" as "numbers", status:"published", category_id:cat.id,
      content_config:cfg, tags, tags_en, quality_score:85, reading_time:2,
      published_at:new Date().toISOString(),
    }).select().single();
    if (err) return NextResponse.json({ error:err.message }, { status:500 });
    // Register topic for deduplication
    await registerTopic(post?.title_ar ?? effectiveTopic, "numbers", category_slug, post?.id);
