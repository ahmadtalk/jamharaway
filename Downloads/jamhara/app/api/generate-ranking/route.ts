...
export async function POST(req: NextRequest) {
  ...
  if (topic?.trim()) {
    const dupCheck = await checkTopicDuplicate(topic.trim(), "ranking", category_slug);
  ...
  const recentTopics = await getRecentTopics(category_slug, "ranking", 15, 30);
  const recentTopicsBlock = recentTopics.length > 0
    ...
    : "";
  const promptText = buildRankingPrompt({
  ...
  try {
    ...
    await registerTopic(post?.title_ar ?? effectiveTopic, "ranking", category_slug, post?.id);
