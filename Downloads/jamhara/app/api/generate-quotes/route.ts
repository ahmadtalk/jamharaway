...
import { buildQuotesPrompt } from "@/lib/prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (topic?.trim()) {
    const dupCheck = await checkTopicDuplicate(topic.trim(), "quotes", category_slug);
  ...
  const recentTopics = await getRecentTopics(category_slug, "quotes", 15, 30);
  const recentTopicsBlock = recentTopics.length > 0
    ...
    : "";
  const promptText = buildQuotesPrompt({
  ...
  try {
    ...
    if (!parsed) {
      console.error("[generate-quotes] JSON parse failed. Raw response (first 600 chars):", resultText.slice(0, 600));
      return NextResponse.json({ error:"JSON parse failed", debug: resultText.slice(0, 200) }, { status:500 });
    }
    const cfg = parsed.content_config;
    if (!cfg?.quotes?.length) return NextResponse.json({ error:"Invalid quotes config" }, { status:500 });
    const { data:post, error:err } = await supabase.from("posts").insert({
      ...
      type:"quotes" as "quotes", status:"published", category_id:cat.id,
      content_config:cfg, quality_score:90, reading_time:3,
      published_at:new Date().toISOString(),
    }).select().single();
    if (err) return NextResponse.json({ error:err.message }, { status:500 });
    // Register topic for deduplication
    await registerTopic(post?.title_ar ?? effectiveTopic, "quotes", category_slug, post?.id);
