...
import { buildTimelinePrompt } from "@/lib/prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (topic?.trim()) {
    const dupCheck = await checkTopicDuplicate(topic.trim(), "timeline", category_slug);
  ...
  const recentTopics = await getRecentTopics(category_slug, "timeline", 15, 30);
  const recentTopicsBlock = recentTopics.length > 0
    ...
    : "";
  const promptText = buildTimelinePrompt({
  ...
  try {
    ...
    if (!cfg?.events?.length) return NextResponse.json({ error:"Invalid timeline config" }, { status:500 });
    ...
    const { data:post, error:err } = await supabase.from("posts").insert({
      ...
      type:"timeline" as "timeline", status:"published", category_id:cat.id,
      content_config:cfg, tags, tags_en, quality_score:85, reading_time:3,
      published_at:new Date().toISOString(),
    }).select().single();
    if (err) return NextResponse.json({ error:err.message }, { status:500 });
    // Register topic for deduplication
    await registerTopic(post?.title_ar ?? effectiveTopic, "timeline", category_slug, post?.id);
