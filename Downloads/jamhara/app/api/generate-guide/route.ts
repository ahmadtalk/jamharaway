...
export async function POST(req: NextRequest) {
  ...
  const effectiveTopic = topic?.trim() || `اختر موضوعاً عملياً من قسم ${cat.name_ar} يمكن شرحه خطوة بخطوة`;
  // Layer 2a — فحص التكرار عندما يُوفَّر topic صريح
  if (topic?.trim()) {
    const dupCheck = await checkTopicDuplicate(topic.trim(), "guide", category_slug);
  ...
  const recentTopics = await getRecentTopics(category_slug, "guide", 15, 30);
  const recentTopicsBlock = recentTopics.length > 0
    ...
    : "";
  const promptText = buildGuidePrompt({
  ...
  try {
    ...
    if (!parsed?.content_config?.steps?.length) return NextResponse.json({ error: "Invalid guide config", raw: resultText.slice(0,300) }, { status: 500 });
    ...
    await registerTopic(post?.title_ar ?? effectiveTopic, "guide", category_slug, post?.id);
