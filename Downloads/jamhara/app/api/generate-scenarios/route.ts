...
export async function POST(req: NextRequest) {
  ...
  if (topic?.trim()) {
    const dupCheck = await checkTopicDuplicate(topic.trim(), "scenarios", category_slug);
  ...
  const recentTopics = await getRecentTopics(category_slug, "scenarios", 15, 30);
  const recentTopicsBlock = recentTopics.length > 0
    ...
    : "";
  const promptText = buildScenariosPrompt({
  ...
  try {
    ...
    if (!cfg?.scenarios?.length) return NextResponse.json({ error:"Invalid scenarios config" }, { status:500 });
    ...
    const { data:post, error:err } = await supabase.from("posts").insert({
      ...
      type:"scenarios" as "scenarios", status:"published", category_id:cat.id,
      content_config:cfg, tags, tags_en, quality_score:85, reading_time:3,
      published_at:new Date().toISOString(),
    }).select().single();
    if (err) return NextResponse.json({ error:err.message }, { status:500 });
    // Register topic for deduplication
    await registerTopic(post?.title_ar ?? effectiveTopic, "scenarios", category_slug, post?.id);
