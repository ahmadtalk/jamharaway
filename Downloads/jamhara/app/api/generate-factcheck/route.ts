...
import { buildFactcheckPrompt } from "@/lib/prompts";
...
export async function POST(req: NextRequest) {
  ...
  if (topic?.trim()) {
    const dupCheck = await checkTopicDuplicate(topic.trim(), "factcheck", category_slug);
  ...
  const recentTopics = await getRecentTopics(category_slug, "factcheck", 15, 30);
  const recentTopicsBlock = recentTopics.length > 0
    ...
    : "";
  const promptText = buildFactcheckPrompt({
  ...
  try {
    ...
    if (use_web_search) {
      ...
      if (hasToolUse || !resultText.trim() || !resultText.includes("{")) {
        resultText = "";
        const f = await anthropic.messages.create({
          model:CONTENT_MODEL, max_tokens:4096,
          messages:[
        ...
        for (const b of f.content) if (b.type==="text") resultText += b.text;
      }
    } else {
      const response = await anthropic.messages.create({
        model:CONTENT_MODEL, max_tokens:4096,
    ...
    if (!cfg?.claims?.length) return NextResponse.json({ error:"Invalid factcheck config" }, { status:500 });
    ...
    const { data:post, error:err } = await supabase.from("posts").insert({
      ...
      type:"factcheck" as "factcheck", status:"published", category_id:cat.id,
      content_config:cfg, tags, tags_en, quality_score:90, reading_time:4,
      published_at:new Date().toISOString(),
    }).select().single();
    if (err) return NextResponse.json({ error:err.message }, { status:500 });
    // Register topic for deduplication
    await registerTopic(post?.title_ar ?? effectiveTopic, "factcheck", category_slug, post?.id);
