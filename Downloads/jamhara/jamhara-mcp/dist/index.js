...
// ─── Supabase ────────────────────────────────────────────────────────────────
const SUPABASE_URL = process.env.SUPABASE_URL || "https://kzxphzobrkgqqshxzvjr.supabase.co";
...
// ─── Content Formatter ───────────────────────────────────────────────────────
function formatPost(post, locale = "ar") {
    const ar = locale === "ar";
    const lines = [];
    const title = ar ? post.title_ar : (post.title_en || post.title_ar);
    lines.push(`# ${title || "—"}`);
    if (post.title_en && post.title_ar && !ar)
        lines.push(`*${post.title_ar}*`);
    lines.push("");
    ...
    switch (post.type) {
        ...
        case "news": {
            ...
            if (lede) {
                lines.push(`## ${ar ? "اللده" : "Lede"}`);
                lines.push(lede);
                lines.push("");
            }
            if (why) {
                lines.push(`## ${ar ? "لماذا يهم؟" : "Why It Matters"}`);
                lines.push(why);
                lines.push("");
            }
            if (pts?.length) {
            ...
            if (next) {
                lines.push(`## ${ar ? "ما التالي؟" : "What's Next"}`);
                lines.push(next);
            }
            break;
        }
        ...
        case "scenarios": {
            ...
            if (question) {
                lines.push(`**${ar ? "التساؤل" : "Question"}:** ${question}`);
                lines.push("");
            }
        ...
        case "debate": {
            const question = ar ? c?.question_ar : (c?.question_en || c?.question_ar);
            if (question) {
                lines.push(`**${ar ? "السؤال" : "Question"}:** ${question}`);
                lines.push("");
            }
            ["side_a", "side_b"].forEach((side) => {
                ...
                lines.push(`## ${label}`);
                c[side].arguments?.forEach((a) => lines.push(`- ${a}`));
                lines.push("");
            });
            const verdict = ar ? c?.verdict_ar : (c?.verdict_en || c?.verdict_ar);
            if (verdict) {
                lines.push(`## ${ar ? "الخلاصة" : "Verdict"}`);
                lines.push(verdict);
            }
            break;
        }
        case "briefing": {
            ...
            if (bl) {
                lines.push(`## ${ar ? "الخلاصة" : "Bottom Line"}`);
                lines.push(bl);
            }
            break;
        }
        case "quotes": {
            ...
            if (topic) {
                lines.push(`**${ar ? "الموضوع" : "Topic"}:** ${topic}`);
                lines.push("");
            }
            if (c?.quotes?.length) {
                c.quotes.forEach((qt) => {
        ...
        case "network": {
            ...
            if (center) {
                lines.push(`**${ar ? "المركز" : "Center"}:** ${center}${role ? ` (${role})` : ""}`);
                lines.push("");
            }
            if (c?.nodes?.length) {
        ...
        case "interview": {
            ...
            if (interviewee) {
                lines.push(`**${ar ? "الضيف" : "Guest"}:** ${interviewee}${role ? `, ${role}` : ""}`);
            }
            if (c?.date)
                lines.push(`**${ar ? "التاريخ" : "Date"}:** ${c.date}`);
            if (context) {
                lines.push("");
                lines.push(context);
            }
        ...
        case "map": {
            ...
            if (topic)
                lines.push(`**${ar ? "الموضوع" : "Topic"}:** ${topic}`);
            if (metric)
                lines.push(`**${ar ? "المقياس" : "Metric"}:** ${metric}`);
            if (insight) {
                lines.push("");
                lines.push(`💡 ${insight}`);
            }
        ...
        case "chart": {
            ...
            if (title)
                lines.push(`**${ar ? "المخطط" : "Chart"}:** ${title}`);
            if (desc)
                lines.push(desc);
...
function createJamharaServer() {
    ...
    server.tool("search_posts", "ابحث في منشورات جمهرة — منصة معرفية عربية تضم 19 نوع محتوى و43 قالباً بصرياً", {
        query: zod_1.z.string().describe("نص البحث (عربي أو إنجليزي)"),
        type: zod_1.z.string().optional().describe(`نوع المحتوى: ${POST_TYPES.join(", ")}`),
        category_slug: zod_1.z.string().optional().describe("slug التصنيف (مثال: politics, technology)"),
        limit: zod_1.z.number().min(1).max(20).default(10).describe("عدد النتائج (افتراضي: 10)"),
        locale: zod_1.z.enum(["ar", "en"]).default("ar").describe("لغة النتائج"),
        ...
        try {
            ...
            if (!data?.length)
                return { content: [{ type: "text", text: `لم يُعثر على نتائج لـ "${query}"` }] };
            ...
            data.forEach((post, i) => {
                ...
                if (post.tags?.length)
                    lines.push(`   🏷️ ${post.tags.slice(0, 4).join(" • ")}`);
                lines.push("");
            });
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `❌ خطأ: ${err.message}` }] };
        }
    });
    // ── Tool 2: get_post ──────────────────────────────────────────────────────
    server.tool("get_post", "احصل على المحتوى الكامل لمنشور من جمهرة بواسطة الـ ID — يعيد النص الكامل بحسب نوع المحتوى", {
        id: zod_1.z.string().describe("معرّف المنشور UUID"),
        locale: zod_1.z.enum(["ar", "en"]).default("ar").describe("لغة المحتوى"),
    }, async ({ id, locale }) => {
        try {
            const { data, error } = await supabase
                ...
                .single();
            if (error || !data)
                return { content: [{ type: "text", text: `لم يُعثر على المنشور: ${id}` }] };
            return { content: [{ type: "text", text: formatPost(data, locale) }] };
        }
        catch (err) {
            return { content: [{ type: "text", text: `❌ خطأ: ${err.message}` }] };
        }
    ...
    server.tool("get_latest_posts", "احصل على أحدث منشورات جمهرة مع إمكانية التصفية بالنوع أو التصنيف", {
        type: zod_1.z.string().optional().describe(`نوع المحتوى: ${POST_TYPES.join(", ")}`),
        category_slug: zod_1.z.string().optional().describe("slug التصنيف"),
        limit: zod_1.z.number().min(1).max(20).default(10).describe("عدد النتائج"),
        ...
        try {
            ...
            if (error)
                throw new Error(error.message);
            if (!data?.length)
                return { content: [{ type: "text", text: "لا توجد منشورات." }] };
            const isAr = locale === "ar";
    ...
    server.tool("list_categories", "احصل على قائمة تصنيفات جمهرة مع عدد منشورات كل تصنيف", {
        locale: zod_1.z.enum(["ar", "en"]).default("ar"),
    }, async ({ locale }) => {
        try {
            const { data, error } = await supabase
            ...
            if (error)
                throw new Error(error.message);
            if (!data?.length)
                return { content: [{ type: "text", text: "لا توجد تصنيفات." }] };
            const isAr = locale === "ar";
    ...
    server.tool("get_statistics", "احصل على إحصائيات منصة جمهرة: إجمالي المنشورات وتوزيعها حسب النوع والتصنيف", {
        locale: zod_1.z.enum(["ar", "en"]).default("ar"),
    }, async ({ locale }) => {
        try {
